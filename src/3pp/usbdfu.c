...
static const char *usb_strings[] = {
	...
	"STM32 custom DFU mode",
	"SN_1",
	/* This string is used by ST Microelectronics' DfuSe utility. */
  // see UM0290 for reference
...
static uint8_t get_sector_number(uint32_t addr) {
  switch (addr) {
    case 0x08000000: // bootloader
      return 0x00;
    case 0x08004000:
      return 0x01;
    case 0x08008000:
...
static void usbdfu_getstatus_complete(usbd_device *usbd_dev, struct usb_setup_data *req)
{
	...
	case STATE_DFU_DNBUSY:
		...
		if (prog.blocknum == 0) {
			...
			case CMD_ERASE:
				{
					uint32_t erase_addr = *(uint32_t *)(prog.buf + 1);
          uint8_t sector = get_sector_number(erase_addr);
          if (sector > 0 && sector < 8)
            flash_erase_sector(sector, 2); /* program_size=32-bit */
				}
				break;
			case CMD_SETADDR:
				{
